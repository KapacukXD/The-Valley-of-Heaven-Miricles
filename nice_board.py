...
class Board:
    ...
    def change_turn(self):
        if self.is_sm_win():
            return None
        self.can_move, self.can_attack = True, False
        self.who += 1
    ...
    def enemy_turn1(self):
        cur_char = self.queue[self.who]
        focus_on = self.get_focus()
        cx, cy, tox, toy = cur_char.x, cur_char.y, focus_on.x, focus_on.y
        gtx, gty = 0, 0
        gtr = 10**9
        if bfs_lite((cy, cx), (toy, tox)) > cur_char.range:
            for row in self.board:
                for cell in row:
                    cell_x, cell_y = cell.x, cell.y
                    if bfs((cy, cx), (cell_y, cell_x)) <= cur_char.step:
                        gtr_n = bfs_lite((cell_y, cell_x), (toy, tox))
                        if gtr_n < gtr:
                            gtx, gty = cell_x, cell_y
                            gtr = gtr_n
            self.move_list = path((cy, cx), (gty, gtx))
        else:
            self.enemy_turn2()

    def enemy_turn2(self):
        cur_char = self.queue[self.who]
        focus_on = self.get_focus()
        cx, cy, tox, toy = cur_char.x, cur_char.y, focus_on.x, focus_on.y
        gtx, gty = 0, 0
        gtr = 10 ** 9
        if bfs_lite((cy, cx), (toy, tox)) <= cur_char.range:
            cur_char.do_attack(focus_on)
        if focus_on.cur_hp <= 0:
            self.board[focus_on.y][focus_on.x].die()
        self.change_turn()
    ...
    def get_focus(self):
        ...
        if win_ch:
            if win_ch == 'a':
                ...
                return stop('enemy')
        for i in sorted(chars, key=lambda ch: ch.cur_hp):
            if i.is_ally and i.cur_hp > 0:
                return i
